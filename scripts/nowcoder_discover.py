...
KEYWORDS = [
    "AI Agent 深圳", "AI应用开发 深圳", "智能体开发 深圳", "AI Agent后端 深圳",
    "大模型应用开发 深圳", "LLM应用开发 深圳", "RAG开发 深圳", "Python AI开发 深圳",
    "AI Agent 面经", "AI应用开发 面经", "大模型应用开发 面经", "RAG 面经",
]
KEEP = re.compile(
    r"(深圳|AI|Agent|智能体|大模型|LLM|RAG|检索增强|LangChain|LangGraph|MCP|"
    r"Python|后端|面经|一面|二面|三面|笔试|内推|招聘|校招|实习|投递)"
)
ACTION = re.compile(
    r"(内推|招聘|招聘信息|岗位|投递|网申|开放|补录|直招|hc|HC|实习生招聘|"
    r"招.*AI|招.*Agent|招.*智能体|招.*大模型|招.*后端|招.*Python)"
)
ROLE_SIGNAL = re.compile(
    r"(AI.?Agent|智能体|AI应用|大模型应用|LLM|RAG|检索增强|LangChain|LangGraph|"
    r"MCP|Python|后端|算法|机器学习|大模型)"
)
NOISE = re.compile(
