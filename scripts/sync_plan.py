...
def _target_cities() -> set[str]:
    """读取画像城市，避免日常任务消耗在无关城市型信源上。"""
    with open(sync.PROFILES_JSON, encoding="utf-8") as f:
        profiles = json.load(f)
    return {str(c).strip() for p in profiles.values() for c in p.get("cities", []) if str(c).strip()}


def _matches_target_city(src: dict[str, str], cities: set[str]) -> bool:
    """全国信源保留；明确只覆盖其它城市或远程的信源跳过。"""
    scope = (src.get("city_scope") or "").lower()
    if not scope or "全国" in scope:
        return True
    return any(city.lower() in scope for city in cities)
...
def _fast_source_ids() -> set[str]:
    ...
    for src in _active_sources():
        ...
        if method == "playwright" or source_type == "community":
            continue
        if not _matches_target_city(src, cities):
            continue
        if _priority(src) <= 2 or sid.startswith("gov-") or sid == "cn-iguopin":
            ids.add(sid)
    catalog = {src["source_id"]: src for src in _active_sources()}
    required = {
        sid for sid in (CORE_SOURCE_IDS | ROLE_SOURCE_IDS)
        if sid in catalog and _matches_target_city(catalog[sid], cities)
    }
    return ids | required
