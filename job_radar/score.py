...
def score_job(job: Job, profile: Dict) -> ScoreResult:
    ...
    target_cities = [normalize_city(c) for c in profile.get("cities", [])]
    if job_city and job_city in target_cities:
        score += 10
        tags.append("city_match")
    elif job_city and profile.get("strict_city"):
        score -= int(profile.get("city_mismatch_penalty", 120))
        tags.append("city_mismatch")
