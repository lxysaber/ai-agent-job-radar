...
TARGET_ROLE_SIGNAL = (
    ...
    "业务分析", "决策支持",
    "ai agent", "agent开发", "agent 后端", "ai应用", "ai 应用", "智能体开发",
    "智能体后端", "大模型应用", "llm应用", "llm 应用", "rag", "检索增强",
    "langchain", "langgraph", "llamaindex", "mcp", "function calling", "tool calling",
)

AGENT_APP_SIGNAL = (
    "ai agent", "agent开发", "agent 后端", "智能体", "ai应用", "ai 应用",
    "大模型应用", "llm应用", "llm 应用", "rag", "检索增强", "langchain",
    "langgraph", "llamaindex", "mcp", "function calling", "tool calling",
)
...
def role_signal_score(title: str, jd_text: str) -> Tuple[int, List[str]]:
    ...
    if any(k in text_low for k in AI_PRODUCT_SIGNAL):
        score += 10
        tags.append("AI产品")
    if any(k in text_low for k in AGENT_APP_SIGNAL):
        score += 26
        tags.append("AI Agent/应用")
